...
- **Upgrading from 1.3.x**: the shared config layer is gone in 1.4.0 — balance no longer hoists skills, MCP servers, settings or memory out of your account dirs, and no longer reads claude.ai connectors. What an earlier version already wrote is left alone: `~/.balance/shared` still holds the hoisted copy, and an account dir may still symlink `skills/`, `agents/`, `commands/` and `plugins/` into it. Those symlinks keep working; replace one with a real directory to give that account its own copy back.
- **Expired login**: refresh tokens are single-use, so on macOS the copy in `<account>/.credentials.json` can fall behind the one Claude Code renewed into the Keychain. balance adopts the Keychain's login when it belongs to that account; when it can't, it says the login expired and stops rather than launching as whoever is in the Keychain. Sign in again in place with `balance account add --name <name>` — the account's Claude Code profile is kept.
...
The suite covers the parts with teeth: config loading and the v0.x migration,
account bookkeeping, argument parsing, and the Keychain blob merge. Every test
runs against its own `BALANCE_HOME` under the system temp dir, so nothing can
reach your real `~/.balance`, and the Keychain merge is tested as a pure
function rather than against the real Keychain.
